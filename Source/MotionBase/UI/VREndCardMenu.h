 ...
 * 종료 화면(UVRResultBoard)에서는 큼직한 버튼 두 장을 두고, 넉넉한 각도로 겨누기만 하면
 * 고를 수 있게 한다. 모드 선택 화면(AModeSelectPawn)이 이미 쓰는 방식과 같은 판정이라
 * 플레이어가 배운 조작이 그대로 통한다.
 *
 * 소유 폰이 상태를 들고, UVRResultBoard::UpdateButtons 가 매 프레임 UpdateTargets() 를 호출한다. (컴포넌트 아님 — POD)
 */
struct FVREndCardMenu
{
	...
	/** 현재 겨누고 있는 카드 인덱스. 없으면 INDEX_NONE. */
	 ...
	 * 겨눔 판정 + 드웰 누적.
	 *
	 * @param CardLocations 카드 중심 월드 위치들 (인덱스 = 카드 번호).
	 * @param AimOrigin     조준 광선의 시작점 (컨트롤러/배트 끝).
	 * @param AimDir        조준 방향 (월드).
	 * @param bTracked      추적 여부. 끊기면 겨눔 없음 + 진행 리셋.
	 * @return 이번 프레임에 확정된 카드 인덱스. 없으면 INDEX_NONE.
	 */
	int32 UpdateTargets(TConstArrayView<FVector> CardLocations,
