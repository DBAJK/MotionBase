...
public:
	 ...
	 *  - **종목별 최근 N회(Config.RecentSessionWindow) 중 최고점**을 쓴다. 한 번 망친 판이 종합을
	 *    바로 깎지 않게 하면서도, 전 역대 최고점을 쓰면 옛 기록(이전 채점식·시도 1~2회짜리 판)이
	 *    최고점에 박혀 **새로 플레이해도 종합이 영영 안 바뀌는** 문제가 생겨 창을 둔다.
	 *  - 시도 수가 Config.MinAttemptsForOverall 미만인 세션, 수비의 **옛 성공률 채점 세션**
	 *    (Details 에 SuccessCount 만 있고 ConsistencySampleCount 가 없음)은 후보에서 뺀다.
	 *  - 종목마다 가장 최근 유효 세션 점수(LatestScore)도 함께 채운다.
